...
void LPUART_ITDMAConfig(uint8_t instance, LPUART_ITDMAConfig_Type config, FunctionalState newState)
{
    ...
    // disable DMA and it
    LPUART_InstanceTable[instance]->C5 &= ~UART0_C5_TDMAE_MASK;
    LPUART_InstanceTable[instance]->C5 &= ~UART0_C5_RDMAE_MASK;
    ...
    (ENABLE == newState)?(NVIC_EnableIRQ((IRQn_Type)(LPUART_IRQBase + instance))):(NVIC_DisableIRQ((IRQn_Type)(LPUART_IRQBase + instance)));
}
...
void UART0_IRQHandler(void)
{
   ...
   uint8_t ch;
   if(LPUART_InstanceTable[HW_LPUART0]->S1 & UART_S1_TDRE_MASK)
   {
