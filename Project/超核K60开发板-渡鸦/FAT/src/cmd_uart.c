...
static void UART_TxISR(uint8_t * pbyteToSend)
{
    *pbyteToSend = gReceicedChar;
    ...
    UART_ITDMAConfig(instance, kUART_IT_Tx, false);
}
...
int CMD_UART(int argc, char * const * argv)
{
    ...
    if(argc == 2 && (!strcmp(argv[1],"DMA")))
    {
        ...
        DelayMs(2); //等待串口空闲
        UART_ITDMAConfig(instance, kUART_DMA_Tx, true); //开启UART0 DMA 发送
        for(i=0;i<10;i++)
        {
            DMA_EnableRequest(1);
            while(DMA_IsMajorLoopComplete(1) == 1);
        }
        UART_ITDMAConfig(instance, kUART_DMA_Tx, false);
    }
    return 0;
}


SHELL_EXPORT_CMD(CMD_UART, UART, UART <instance>);