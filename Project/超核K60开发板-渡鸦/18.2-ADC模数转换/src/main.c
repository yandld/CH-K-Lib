...
int main(void)
{
    ...
    ADC_InitStruct1.resolutionMode = kADC_SingleDiff10or11;  //设置转换精度
    ...
    while(1)
    {
        ...
        if(ADC_IsConversionCompleted(HW_ADC0, kADC_MuxA) == 0)
        {
            ...
            printf("ADC[19]:%04d\r", ADC_ReadValue(HW_ADC0, kADC_MuxA));//获取ad通道的转换结果
        }
        DelayMs(5);
    }
}
