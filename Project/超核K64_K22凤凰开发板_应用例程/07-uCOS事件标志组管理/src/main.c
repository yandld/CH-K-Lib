 ...
 *					2.按下凤凰开发板的KEY1按钮，这时候会打印taskLedOn
 * 说明：  
...
static void PTA4_EXTI_ISR(uint32_t pinxArray);
...
int main(void)
{
	/* 硬件初始化 */
	uint32_t instance;	
	instance = GPIO_QuickInit(HW_GPIOA, 4, kGPIO_Mode_IPU);
    GPIO_ITDMAConfig(HW_GPIOA, 4, kGPIO_IT_FallingEdge, true);
    GPIO_CallbackInstall(instance, PTA4_EXTI_ISR);
